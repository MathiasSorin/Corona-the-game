...
#Images/Sound/Font.
player1image = pygame.image.load("player.png")
proteinimage = pygame.image.load("protein.png")
    ...
    while gameover:
        for event in pygame.event.get():
            ...
            if event.type == 28:
                ...
                draw_text(mainwindow, "CORONAVIRUS", 80, 500, 300)
                draw_text(mainwindow, "THE GAME", 60, 500, 450)
                draw_text(mainwindow, "PRESS ENTER TO START", 30, 500, 600)
...
def gameover():
    ...
    while gameover:
        for event in pygame.event.get():
            ...
            if event.type == 28:
                mainwindow.fill((0, 0, 0))
                draw_text(mainwindow, "GAME OVER", 80, 500, 300)
                draw_text(mainwindow, "Your Score: {}".format(str(score)), 30, 500, 400)
                draw_text(mainwindow, "PRESS ENTER TO RETRY", 30, 500, 500)
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    gameover = False
                    running = True
...
#Timer event, make something happen after x.
...
#Slow/Unslow time, checks if slow event is already ongoing.
def slowtime(state):
    global fps
    dupcheck = False
    if state:
        ...
        if timeeventlist:
            for events in timeeventlist:
                if timeevent.timer(events):
                    #If slow event finishes, check for other slow events in list.
                    if events.reason == "slow":
                        slowtime(False)
